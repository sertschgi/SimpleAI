...
pub fn Viewport() -> Element {
    // ------------------------------ VARIABLES ------------------------------ //
    let mut node_container = ViewportNodeContainer::new();

    let fnc = node_container.frontend_node_container;
    ...
    let mousedown = move |e: MouseEvent| {
        for mut node in node_container.frontend_node_container.iter_mut() {
    ...
    let mouseup = move |e: MouseEvent| {
        ...
        if let Some(node) = crate::global::context::DRAG_NODE() {
            node_container.push_context(node.clone());
        }
    };
